...
def test_create_contract_controller(mock_session):
    with patch(
        "controllers.contract_controller.SessionLocal",
        return_value=mock_session,
    ), patch(
        "controllers.contract_controller.validate_create_contract_input",
        return_value=MagicMock(dict=lambda: {}),
    ), patch(
        "controllers.contract_controller.Contract"
    ) as mock_contract, patch(
        "controllers.contract_controller.success_create_contract_view"
    ) as mock_success_view:

        create_contract_controller(1, 2, 1000, 500, True)
...
def test_list_contracts_controller(mock_session):
    with patch(
        ...
        "controllers.contract_controller.Contract.get_all"
    ) as mock_get_all, patch(
        "controllers.contract_controller.list_contracts_view"
    ) as mock_list_view:
...
def test_delete_contract_controller(mock_session):
    with patch(
        ...
        "controllers.contract_controller.Contract.get_by_id",
        return_value=MagicMock(id=1),
    ) as mock_get_by_id, patch(
        "controllers.contract_controller.success_delete_contract_view"
    ) as mock_success_view:
...
def test_delete_contract_controller_contract_not_found(mock_session):
    with patch(
        ...
        "controllers.contract_controller.Contract.get_by_id", return_value=None
    ) as mock_get_by_id, patch(
        "controllers.contract_controller.success_delete_contract_view"
    ) as mock_success_view, patch(
        "controllers.contract_controller.error_contract_not_found_view"
    ) as mock_error_view:
...
def test_update_contract_controller(mock_session):
    with patch(
        ...
        "controllers.contract_controller.validate_update_contract_input",
        return_value=MagicMock(dict=lambda: {}),
    ), patch(
        "controllers.contract_controller.Contract.get_by_id",
        return_value=MagicMock(id=1),
    ) as mock_get_by_id, patch(
        "controllers.contract_controller.Client.get_by_id",
        return_value=MagicMock(id=2),
    ) as mock_get_client_by_id, patch(
        "controllers.contract_controller.Collaborator.get_by_id",
        return_value=MagicMock(id=3),
    ) as mock_get_collaborator_by_id, patch(
        "controllers.contract_controller.success_update_contract_view"
    ) as mock_success_view, patch(
        "controllers.contract_controller.error_contract_not_found_view"
    ) as mock_error_contract_view, patch(
        "controllers.contract_controller.error_client_collaborator_not_found_view"
    ) as mock_error_client_collaborator_view:

        update_contract_controller(1, 2, 3, 1000, 500, True)
...
def test_update_contract_controller_client_not_found(mock_session):
    with patch(
        ...
        "controllers.contract_controller.Client.get_by_id", return_value=None
    ) as mock_get_client_by_id, patch(
        "controllers.contract_controller.error_contract_not_found_view"
    ) as mock_error_contract_view, patch(
        "controllers.contract_controller.error_client_collaborator_not_found_view"
